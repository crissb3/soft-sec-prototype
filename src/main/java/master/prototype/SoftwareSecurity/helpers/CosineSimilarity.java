...
public class CosineSimilarity {
	...
	private String security_src;
	
	private Map<String, Integer> tfreq_quiz;
	private Map<String, Integer> tfreq_security_src;
	
	private double cosineSimilarity;
	
	
	public CosineSimilarity(String answer, String search_results) {
		quiz_answer = answer;
		security_src = search_results;
		tfreq_quiz = new HashMap<>();
		tfreq_security_src = new HashMap<>();
		vectorize();
		computeCosineSimilarity();
	}
	
	// we vectorize here by counting the number of occurrence of each term in the "feature"
	// that we can find in our "text"
	private void createVectors(List<String> features, String text, Map<String, Integer> tfreq){
		
		for(String s : features) {
			for(String t : text.split(" ")) {
				t = t.trim().toLowerCase();
				if(!getStopwords().contains(t)) {
					if(t.contains(s)) {
						int value = tfreq.get(s) + 1;
						tfreq.put(s, value);
					} 
				}
			}
	...
	// the quiz_answer text will be tokenize and the tokens will be used as the feature list
	private List<String> createFeatures(){
		Set<String> tokens = new HashSet<>();
		for(String t : quiz_answer.split(" ")) {
			t = t.trim().toLowerCase();
			if(!getStopwords().contains(t)) {
				tokens.add(t);
			}
		...
		for(String s : tokens) {
			tfreq_quiz.put(s, 0);			//initialize the tfreq map for quiz
			tfreq_security_src.put(s, 0);	//initialize the tfreq map for quiz
		}
	...
	private void vectorize() {
		List<String> features = createFeatures();
		createVectors(features, quiz_answer, tfreq_quiz);  			// create vectors for quiz_answer
		createVectors(features, security_src, tfreq_security_src);  // create vectors for security_src
	}
	
	private void computeCosineSimilarity() {

		// cosine(quiz, security_src) = ai.bi/||ai||x||bi||
		double dotp = 0, suma = 0, sumb = 0;
		//for(int i=1; i<tfreq_quiz.size(); i++) {
		for(String term : tfreq_quiz.keySet()) {
			
			double cval1 = tfreq_quiz.get(term);
			double cval2 = tfreq_security_src.get(term);
			
			//System.out.println(i+" | "+cval1+" | "+cval2);
			suma += Math.pow(cval1, 2);
			sumb += Math.pow(cval2, 2);
			dotp += cval1*cval2;
		}
		suma = Math.sqrt(suma);
		sumb = Math.sqrt(sumb);
		...
		double cosdist = dotp/(suma*sumb);
		if(Double.isNaN(cosdist))
			cosdist = 0;
		
		//System.out.println(cosdist+" |"+dotp+" |"+suma+" |"+sumb);
		
		cosineSimilarity = cosdist;
	}
	
	// manually include the stopwords
	private List<String> getStopwords(){
		...
		return Arrays.asList(
				...
				"i2", "i3", "i4", "i6", "i7", "i8", "ia", "ib", "ibid", "ic", "id", "i'd", "ie", "if", "ig", "ignored",
				"ih", "ii", "ij", "il", "i'll", "im", "i'm", "immediate", "immediately", "importance", "important",
				"in", "inasmuch", "inc", "indeed", "index", "indicate", "indicated", "indicates", "information",
				"inner", "insofar", "instead", "interest", "into", "invention", "inward", "io", "ip", "iq", "ir", "is",
				"isn", "isn't", "it", "itd", "it'd", "it'll", "its", "it's", "itself", "iv", "i've", "ix", "iy", "iz",
				"j", "jj", "jr", "js", "jt", "ju", "just", "k", "ke", "keep", "keeps", "kept", "kg", "kj", "km",
				"know", "known", "knows", "ko", "l", "l2", "la", "largely", "last", "lately", "later", "latter",
				"latterly", "lb", "lc", "le", "least", "les", "less", "lest", "let", "lets", "let's", "lf", "like",
				"liked", "likely", "line", "little", "lj", "ll", "ll", "ln", "lo", "look", "looking", "looks", "los",
				...
				"needn't", "needs", "neither", "never", "nevertheless", "new", "next", "ng", "ni", "nine", "ninety",
				"nj", "nl", "nn", "no", "nobody", "non", "none", "nonetheless", "noone", "nor", "normally", "nos",
				"not", "noted", "nothing", "novel", "now", "nowhere", "nr", "ns", "nt", "ny", "o", "oa", "ob",
				"obtain", "obtained", "obviously", "oc", "od", "of", "off", "often", "og", "oh", "oi", "oj", "ok",
				"okay", "ol", "old", "om", "omitted", "on", "once", "one", "ones", "only", "onto", "oo", "op", "oq",
				"or", "ord", "os", "ot", "other", "others", "otherwise", "ou", "ought", "our", "ours", "ourselves",
				"out", "outside", "over", "overall", "ow", "owing", "own", "ox", "oz", "p", "p1", "p2", "p3", "page",
				...
				"sz", "t", "t1", "t2", "t3", "take", "taken", "taking", "tb", "tc", "td", "te", "tell", "ten", "tends",
				"tf", "th", "than", "thank", "thanks", "thanx", "that", "that'll", "thats", "that's", "that've", "the",
				...
				"tries", "truly", "try", "trying", "ts", "t's", "tt", "tv", "twelve", "twenty", "twice", "two", "tx",
				...
				"whim", "whither", "who", "whod", "whoever", "whole", "who'll", "whom", "whomever", "whos", "who's",
				"whose", "why", "why's", "wi", "widely", "will", "willing", "wish", "with", "within", "without", "wo",
				"won", "wonder", "wont", "won't", "words", "world", "would", "wouldn", "wouldnt", "wouldn't", "www",
				"x", "x1", "x2", "x3", "xf", "xi", "xj", "xk", "xl", "xn", "xo", "xs", "xt", "xv", "xx", "y", "y2",
	...
	public void print() {
		...
		System.out.println(tfreq_quiz.keySet());
		System.out.println();
		System.out.println("=====quiz_answer vector=====");
		System.out.println(tfreq_quiz);
		System.out.println();
		System.out.println("=====security_src vector=====");
		System.out.println(tfreq_security_src);
		System.out.println();
	}
